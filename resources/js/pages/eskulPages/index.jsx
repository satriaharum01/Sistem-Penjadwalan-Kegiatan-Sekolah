...
function EskulPage() {
	return (
		<>
			...
			<Stack spacing={5}>
				<DataTableSection name="Eskul" props={{ dense: true }} />
			</Stack>
		</>
	);
}
...
function DataTableSection({ name, props }) {
	...
	const deletePost = async (id) => {
		await api.delete(`/eskul/delete/${id}`).then(() => {});
	...
	const fetchDataList = async () => {
		try {
			const response = await api.get('/eskul/get'); // Ganti dengan endpoint yang sesuai
			...
			<DataTable
				...
				render={(row) => (
					<TableRow hover tabIndex={-1} key={row.id}>
						...
						<TableCell align="left">{row?.nama_eskul}</TableCell>
						<TableCell align="left">{row?.pembina}</TableCell>
						<TableCell align="left">{row?.pelatih}</TableCell>
						<TableCell align="left">{row?.hari}</TableCell>
						<TableCell align="left">{row?.ruangan}</TableCell>
						<TableCell align="right" sx={{ width: 150 }}>
							...
							<Tooltip title="Edit Data" arrow>
								<IconButton
									...
									size="small"
									sx={{ fontSize: 2 }}
									onClick={() => navigate(`../eskul/edit/${row.id}`)}
								>
