...
function AgendaPage() {
	return (
		<>
			...
			<Stack spacing={5}>
				<DataTableSection name="Agenda" props={{ dense: true }} />
			</Stack>
		</>
	);
}
...
function DataTableSection({ name, props }) {
	...
	const deletePost = async (id) => {
		await api.delete(`/agenda/delete/${id}`).then(() => {});
	...
	const fetchDataList = async () => {
		try {
			const response = await api.get('/agenda/get'); // Ganti dengan endpoint yang sesuai
						...
						<TableCell align="right" sx={{ width: 150 }}>
							...
							<Tooltip title="Edit Data" arrow>
								<IconButton
									...
									size="small"
									sx={{ fontSize: 2 }}
									onClick={() => navigate(`../agenda/edit/${row.id}`)}
								>
